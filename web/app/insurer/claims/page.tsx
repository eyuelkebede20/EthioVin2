...
const INCIDENT_TYPES = ["accident", "theft", "fire", "flood", "vandalism", "total_loss", "other"];
...
export default function ClaimsPage() {
  ...
  const [payoutBand, setPayoutBand] = useState("");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await insurerApi.submitClaim({
        vin,
        incidentType,
        severityBand: severity,
        incidentDate: date ? new Date(date).toISOString() : undefined,
        payoutBand: payoutBand.trim() || undefined,
      });
      setMsg("Claim signal recorded. Thank you for contributing.");
      setVin(""); setDate(""); setPayoutBand("");
    } catch (e2) {
  ...
  return (
    <AppShell title="Insurer" nav={INSURER_NAV}>
      <h1 className="text-title text-fg">Submit a claim signal</h1>
      ...
      <div className="mt-3 flex items-start gap-3 rounded-lg border border-brand-200 bg-brand-50 p-4">
        ...
        <p className="text-body text-fg-muted">Only the <strong className="text-fg">incident type, severity and date</strong> are stored — used to grade the vehicle&apos;s health. No claim narrative, amounts, documents or personal data are kept.</p>
      </div>
      ...
      <form onSubmit={submit} className="card mt-5 max-w-lg space-y-4 p-6">
        ...
        <label className="block text-caption font-bold uppercase tracking-wide text-fg-muted">
          Severity (1 minor – 5 severe): <span className="text-brand-700">{severity}</span>
        ...
        <label className="block text-caption font-bold uppercase tracking-wide text-fg-muted">
          Payout band (optional, e.g. &quot;low&quot;)
          <input value={payoutBand} onChange={(e) => setPayoutBand(e.target.value)} className={inputCls} maxLength={20} />
        </label>
